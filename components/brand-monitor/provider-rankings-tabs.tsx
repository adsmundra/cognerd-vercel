...
export function ProviderRankingsTabs({
  ...
  useEffect(() => {
    if (providerRankings && providerRankings.length > 0) {
      if (!selectedProvider || !providerRankings.find(p => p.provider === selectedProvider)) {
        setSelectedProvider(providerRankings[0].provider);
      }
    }
  }, [providerRankings, selectedProvider]);

  if (!providerRankings || providerRankings.length === 0) return null;

  // Use the first provider if state is not set yet
  const currentProvider = selectedProvider || providerRankings[0].provider;
  ...
  const selectedProviderData = providerRankings.find(p => p.provider === currentProvider);
  ...
  return (
    <Card className="p-2 bg-card text-card-foreground gap-6 rounded-xl border py-6 shadow-sm border-gray-200 h-full flex flex-col">
      ...
      <CardContent className="pt-6 pb-2 flex-1 flex flex-col">
        <Tabs value={currentProvider} onValueChange={setSelectedProvider} className="flex-1 flex flex-col">
          <TabsList 
            className="grid w-full mb-2 h-14" 
            style={{ gridTemplateColumns: `repeat(${Math.min(providerRankings.length, 6)}, minmax(0, 1fr))` }}
          >
